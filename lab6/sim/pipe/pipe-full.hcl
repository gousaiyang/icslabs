...
# Modifications:
# 1. Implement iaddl and leave instructions to reduce the number of instructions.
# 2. Implement load forwarding to eliminate stall between mrmovl and rmmovl/pushl.
# 3. Reduce stalls in pushl/ret condition(pushl followed by ret). Forward valA from memory, 3 stalls in ret can be reduced to only 1 stall.
...
int f_pc = [
	...
	W_icode == IRET : W_valM;
	# pushl/ret condition
	E_icode == IRET && M_icode == IPUSHL : M_valA;
...
int e_valA = [
	E_icode in { IRMMOVL, IPUSHL } && E_srcA == M_dstM : m_valM;	# Forward valA from memory
	1 : E_valA;    # Use original valA
];
...
bool F_stall =
	...
	# but do not stall in load forwarding condition
	(E_icode in { IMRMOVL, IPOPL } && E_dstM in { d_srcA, d_srcB } &&
	!(D_icode in { IRMMOVL, IPUSHL } && d_srcA == E_dstM && d_srcB != E_dstM)) ||
	# Stalling at fetch while ret passes through pipeline
	# but only stall once in pushl/ret condition
...
bool D_bubble =
	...
	# only stall once in pushl/ret condition
	(!(E_icode in { IMRMOVL, IPOPL } && E_dstM in { d_srcA, d_srcB }) &&
	  IRET in { D_icode, E_icode, M_icode } &&
	!((E_icode == IRET && M_icode == IPUSHL) || (M_icode == IRET && W_icode == IPUSHL)));
