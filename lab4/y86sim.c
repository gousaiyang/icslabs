...
stat_t nexti(y86sim_t *sim)
{
    ...
    if (ifun < 0 || ifun > 6 || (icode == I_ALU && ifun > 3) 
        || (icode != I_ALU && icode != I_JMP && icode != I_RRMOVL && ifun != 0)) {
    ...
    if (icode == I_RRMOVL || icode == I_IRMOVL || icode == I_RMMOVL || icode == I_MRMOVL
        || icode == I_ALU || icode == I_PUSHL || icode == I_POPL) {
        byte_t regAB = 0;
        ...
        regA = GET_REGA(regAB);
        regB = GET_REGB(regAB);
        if (icode == I_RRMOVL || icode == I_RMMOVL || icode == I_MRMOVL || icode == I_ALU) {
            if (!NORM_REG(regA) || !NORM_REG(regB)) {
                err_print("PC = 0x%x, Invalid register id %.2x", sim->pc, regAB);
                return STAT_INS;
            }
        }
        else if (icode == I_IRMOVL) {
            if (!NONE_REG(regA) || !NORM_REG(regB)) {
                err_print("PC = 0x%x, Invalid register id %.2x", sim->pc, regAB);
                return STAT_INS;
            }
        }
        else if (icode == I_PUSHL || icode == I_POPL) {
    ...
    switch (icode) {
      ...
      case I_RMMOVL: /* 4:0 regA:regB imm */
        ...
        if (!set_long_val(sim->m, valB + valC, valA)) {
            err_print("PC = 0x%x, Invalid data address %p", sim->pc, (void*)(valB + valC));
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      case I_MRMOVL: /* 5:0 regB:regA imm */
        valB = get_reg_val(sim->r, regB);
        if (!get_long_val(sim->m, valB + valC, &valM)) {
            err_print("PC = 0x%x, Invalid data address %p", sim->pc, (void*)(valB + valC));
            return STAT_ADR;
        }
      ...
      case I_JMP: /* 7:x imm */
        ...
        break;
      case I_CALL: /* 8:0 imm */
        valB = get_reg_val(sim->r, REG_ESP) - 4;
        set_reg_val(sim->r, REG_ESP, valB);
        if (!set_long_val(sim->m, valB, next_pc)) {
            err_print("PC = 0x%x, Invalid stack address %p", sim->pc, (void*)valB);
            return STAT_ADR;
        }
        sim->pc = valC; 
        break;
      case I_RET: /* 9:0 */
        ...
        set_reg_val(sim->r, REG_ESP, valB + 4);
        sim->pc = valM;
        break;
      case I_PUSHL: /* A:0 regA:F */
        ...
        valB = get_reg_val(sim->r, REG_ESP) - 4;
        set_reg_val(sim->r, REG_ESP, valB);
        if (!set_long_val(sim->m, valB, valA)) {
            err_print("PC = 0x%x, Invalid stack address %p", sim->pc, (void*)valB);
            return STAT_ADR;
        }
        sim->pc = next_pc;
